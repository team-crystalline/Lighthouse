...
const { isLoggedIn, getCookies, encryptWithAES, decryptWithAES, forbidUser, 
  lostPage, idCheck, checkUUID, base64decode, getSystems, authUser, validateParam, 
    ...
    router.get("/archive-alter/:id", authUser, validateParam('id'), async (req, res, next)=>{
     ...
     router.get("/del-mood/:id", authUser, validateParam('id'), async (req, res)=>{
      ...
      router.post('/alter/:id/delete', authUser, validateParam('id'), async function (req, res){
          ...
          await db.query(client, "DELETE FROM posts WHERE p_id=$1", [`${req.params.id}`], res, req);
          await db.query(client, "DELETE FROM journals WHERE alt_id=$1;", [`${req.params.id}`], res, req);
          await db.query(client, "DELETE FROM alters WHERE alt_id=$1;", [`${req.params.id}`], res, req);
          
          res.redirect(`/system/${req.body.sysid}`);
      });

      router.post("/alter/:id", authUser, validateParam('id'), async function(req, res){
        if (!checkUUID(req.params.id)) return lostPage(res, req);
          let pass= req.body.jPass || null;
            ...
            if (req.body.create){
              // Create
              await db.query(client, "INSERT INTO journals (alt_id, password, is_private, skin, sys_id) VALUES ($1, $2, $3, $4, $5)", [`${req.params.id}`, `'${CryptoJS.SHA3(req.body.jPass)}'`, `${req.body.priv}`, `'${req.body.journ}'`, `${req.body.sys_id}`], res, req);
              res.redirect(`/alter/${req.params.id}`);
